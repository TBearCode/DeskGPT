...
%type <node> CommandList Command Make Show Goto Where Match Uniques Read Evaluate Open Output Run Count Take Rename Remove WSubject CSubject RSubject ShowOpts ShowArgs ShowOpt ShowArg Location COpt OutArg MatchOpts GLoc MatchOpt ULoc UniqueOpts UniqueOpt ReadLoc OpenLoc TakeOpt
...
CommandList : Command {
	    ...
	    { $$ = ASTCreateNode(A_COMMAND_LIST);
	      $$->s1 = $1;
	      $$->s2 = $3;}
	    | Command T_AND T_PIPE CommandList
	    ;
...
OutArg : T_ALL { $$= ASTCreateNode(A_OUT); $$->name = "*";}
       ...
       | T_LAST { $$ = ASTCreateNode(A_OUT); $$->name = " | "; }
       | T_LAST_FILE { $$ = ASTCreateNode(A_OUT); $$->name = " | xargs -n 1 "; }
       ;

Run : T_BASH T_IN { $$ = ASTCreateNode(A_RUN); $$->name = $2;}
    ;

Count : T_WC COpt CSubject { $$ = $3; $$->s1 = $2;}
      | T_WC CSubject { $$ = $2;}
      ;
...
CSubject : T_LAST {$$ = ASTCreateNode(A_COUNT); $$->name = " | ";}
	 | T_LAST_FILE {$$ = ASTCreateNode(A_COUNT); $$->name = " | xargs -n 1 ";}
	 | T_IN{$$ = ASTCreateNode(A_COUNT); $$->name = $1;}
	 | T_ALL{$$ = ASTCreateNode(A_COUNT); $$->name = "*" ;}
	 ;

Take : T_TAKE T_NUM T_IN T_SEP T_BY TakeOpt {$$ = $6; $$->value = $3;}
     ;

TakeOpt : T_WHITE { $$ = ASTCreateNode(A_TAKE); $$->name = "w";}
